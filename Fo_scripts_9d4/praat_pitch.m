...
%     wavwrite(data, varargin{1}, 16, 'praat.wav');
    ...
    if isunix == 1; commstring = './praat_barren praat_pitch.psc praat.wav';
	elseif ispc == 1; commstring = 'praatcon praat_pitch.psc praat.wav';
	end
%	commstring = 'praatcon praat_pitch.psc praat.wav';
...
%     wavwrite(data, varargin{1}, 16, 'praat.wav');
    ...
    commstring = 'praat praat_pitch.psc praat.wav';
%     commstring = 'praatcon_win98 praat_pitch.psc praat.wav';    
    [s, w] = system(['cd ' varargin{2} ' & ' commstring],'-echo');
    delete([varargin{2} '\praat.wav']);
elseif (nargin == 1)
    if (isa(data,'char'))
        commstring = ['praatcon praat_pitch.psc ' data];
        commstring = ['praatcon_win98 praat_pitch.psc ' data];
    else
