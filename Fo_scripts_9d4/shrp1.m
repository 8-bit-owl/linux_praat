function [f0_time,f0_value,SHR,f0_candidates,enrgyabove,hrmnicenergy]=...
  shrp1(Y,Fs,F0MinMax,frame_length,timestep,SHR_Threshold,ceiling,med_smooth,CHECK_VOICING)
...
%       enrgyabove:     the energy above the predicted pitch / energy below (added by ejh)
%       hrmnicenergy:   ratio of harmonic energy to overal energy (added by ejh)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
interp_logf2=logf(1):min_bin:log2(7000);
logf2=logf2(logf2<log2(7000));
...
newfre2=2.^(interp_logf2); %added by ejh
...
for n=1:nf
    ...
    Spectra = abs(Spectra(2:fftlen/2+1)); 
    interp_Spectra=interp1(logf2,Spectra(1:length(logf2)),interp_logf2,'linear');
    interp_Spectra=interp_Spectra-min(interp_Spectra);    
    energy_aboveFo=sum(interp_Spectra(interp_logf2>log2(curf0))); % added by ejh
    energy_belowFo=sum(interp_Spectra(interp_logf2<log2(curf0))); % added by ejh
    nh=curf0*[1:10]; nh=nh(nh<6900);
    hrmics=[];
    for n1=1:length(nh)
      tmp=find(interp_logf2>log2(nh(n1)));
      hrmics=[hrmics tmp(1)];
      clear tmp
    end
    if hrmics(end)>length(interp_logf2)-3,hrmics=hrmics(1:end-1);end
    HmncEnergy=sum(interp_Spectra(hrmics))...
      +sum(interp_Spectra(hrmics+1))+sum(interp_Spectra(hrmics-1))...
      +sum(interp_Spectra(hrmics+2))+sum(interp_Spectra(hrmics-2))...
      +sum(interp_Spectra(hrmics+3))+sum(interp_Spectra(hrmics-3));
    
    enrgyabove(n)=energy_aboveFo/energy_belowFo;
    hrmnicenergy(n)=HmncEnergy./sum(interp_Spectra);
