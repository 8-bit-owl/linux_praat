...
function longspec=ltas3(x,Fs,Nfft,win,allspectra,sigout,overlap,noiselev,LTAS_lower,LTAS_upper,Fo)
...
% 2021 June, updated how some of the parameters were calculated
...
longspec.AlphaRatio=...
   sum(longspec.dBspectrum((longspec.f>1000&longspec.f<=5000)))/sum((longspec.f>1000&longspec.f<5000))-...
   sum(longspec.dBspectrum((longspec.f>50&longspec.f<=1000)))/sum((longspec.f>50&longspec.f<1000));
longspec.dB1kTo3k=...       % energy in the range normalized to the number of components
   sum(longspec.dBspectrum((longspec.f>=1000&longspec.f<=3150)))...
   /sum(longspec.f>=1000&longspec.f<=3150);

longspec.dB1kTo3kN=...      % energy in the range compared to energy from 50Hz-10Khz
   sum(longspec.dBspectrum((longspec.f>=1000&longspec.f<=3150)))...
   /sum(longspec.dBspectrum((longspec.f>=LTAS_lower&longspec.f<=LTAS_upper)));

longspec.dBsingersform=...   % energy in the range normalized to the number of components
   sum(longspec.dBspectrum((longspec.f>=2800&longspec.f<=3400)))...
...
tmpf=longspec.dBspectrum-longspec.dBspect_slopeFo*longspec.f;
tmp=length(longspec.dBspectrum(longspec.f<=LTAS_upper));
