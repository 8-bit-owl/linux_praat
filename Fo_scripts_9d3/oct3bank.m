...
%
%    [P,F] = OCT3BANK(X,Fs) returns two length-18 row-vectors with 
...
ff = (1000).*((2^(1/3)).^[-10:12]); 	% Exact center freq. 	
P = zeros(1,18);
m = length(x);
...
% Convert to decibels. 
%%%%%%%%% Pref = .00000000001; 				% Reference level for dB scale.  
Pref = 20e-6;
idx = (P>0);
