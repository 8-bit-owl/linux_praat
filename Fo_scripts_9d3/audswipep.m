function [p,t,s,pc,S] = audswipep(x,fs,plim,dt,dlog2p,dERBs,woverlap,sTHR)
% AUDSWIPEP Pitch estimation using Aud-SWIPE'.
%    P = AUDSWIPEP(X,Fs,[PMIN PMAX],DT,DLOG2P,DERBS,WOVERLAP,STHR) 
...
%    However, this may not be recommendable since Aud-SWIPE' is reported to 
%    produce on average better results than Aud-SWIPE (Camacho,2011).
%
...
%       ylabel('Pitch (Hz)')
%
%    NOTE: The method ERBFilters(Fs,FC) used here produces the coefficients
%    of a gammatone filterbank with central frequencies FC, given a
%    sampling frequency FS. The method can be built by modifying the method
...
if ~ exist( 'dlog2p', 'var' ) || isempty(dlog2p), dlog2p = 1/24; end
