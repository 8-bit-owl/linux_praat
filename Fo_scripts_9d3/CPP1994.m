...
attenuationRate = -1; % dB/ms, exponential window decay rate
...
% Create spectrum smoothing filter (removes isolated zeros in magnitude spectrum):
Hsmooth = [1 2 1]'/4; % COLUMN vector

% Create exponential weighting function:
B = 10^(attenuationRate*1e3/fs/20);
W = B.^[0:parameters.windowLength-1]'; % COLUMN vector
...
while p0-1+max(parameters.windowLength,T10) <= length(x)
   ...
   % Spectrum magnitude:
   Xabs = abs(fft(xFrame(:).*W,parameters.FFTsize)); % COLUMN vector
