function [pc,F1,F2,F3,F4,Voice,Pitch,Gender,Filter_Cutoff,Log_ratio_threshold1,...
    Log_ratio_threshold2,Autocorrelation_Threshold_Level] = rfpc(X,Fs)
% FORMANT_TRACKER
% 
%  [F1, F2, F3, F4, Voice, Pitch, Gender, Filter_Cutoff,
%  Log_ratio_threshold1, Log_ratio_threshold2, Autocorrelation_Threshold_Level] = mb_ftracker(X,Fs);
%
%  This function estimates the first four formant frequencies of a speech signal, X and 
%  plots the results on the narrowband spectrogram of X. X has to be passed to the function in vector format. 
%  The sampling frequency of the speech signal has to be specified in Hz, as Fs. 
%
%  The function returns the first four formant frequencies: F1, F2, F3, and F4 (in Hz).
%  
%
% INPUTS
...
[F,Voice,Pitch,Gender,Filter_Cutoff,Log_ratio_threshold1,Log_ratio_threshold2,Autocorrelation_Threshold_Level]...
   = formant_tracker_backend(X,Fs,lpc_window_size,window,Filter_Cutoff_init,Log_ratio_threshold1_init,...
   Log_ratio_threshold2_init,Autocorrelation_Threshold_Level_init,RMS_Ratio_F1,RMS_Ratio_F2,RMS_Ratio_F3,RMS_Ratio_F4);
...
kor = xcorr(F2);
kor = kor/max(kor);
zac = round(length(kor)/2);
kor2 = kor(zac:end);
kor = kor(zac:zac + zac/3);
[pks,lcs] = findpeaks(kor);
[pc,b] = max(pks);

% plot of the first autocorrelation coefficient
%pom = lcs(b);
%sp(pom) = 1.1;
%figure(), plot(kor2)
%hold on
