...
% 2020-Nov par9d3b, fixed some errors around the final plots
% 2021-Feb par9d3c, fixed some errors around trim 4
% 2021-Jun par9d4, updated how LTAS calculates some of the output parameters.
...
u_ftitle = 'SressMales4_modpar'; %'% name to put in documents, no spaces';
...
u_trimYN = 3;         % 0 to analyze the files as they are
...
u_concatYN = 0;     % 2 to concatenate and also save the concatenated wave file
...
fprintf(g_fid_out,'%6s %12s\n','fileNum','fileName');
fclose(g_fid_out); %close file
delete([g_titlename '_failed.txt']);
g_fid_out = fopen([g_titlename '_failed.txt'],'w');
fprintf(g_fid_out,'%6s %12s\n','fileNum','fileName');
fclose(g_fid_out); %close file
delete([g_titlename '_trackfailed.txt']);
g_fid_out = fopen([g_titlename '_trackfailed.txt'],'w');
fclose(g_fid_out); %close file

%% 1_______________ primary loop _____________
g_iter2=0;  % for file output
...
% parfor g_iter = 728:740          %loop through all files in the directory
   %%
   try
   [outdata,sigaS,t1S,sig_dB,t2,t2Si,dB_vals,f0_So_aud,f0_fo_shp,f0_fo_prtf,f0_fo_aud,...
      g_duration]=...
      a_9d4_par_task_function(g_iter,g_cnt,g_filename,g_titlename,...
      s_gen,s_chnnum,s_gennum,u_avqi,s_gain_voltA,s_code,s_files,...
...
%       fprintf(g_fid_out,'-%d\n',g_iter); %write out title
      fprintf(g_fid_out,'%6d %s\n',g_iter,g_filename{g_iter}); %write out successful file
      fclose(g_fid_out); %close file     
   end
   catch  
...
%       fprintf(g_fid_out,'%s\n',g_filename{g_iter}); %write out title
      fprintf(g_fid_out,'%6d %s\n',g_iter,g_filename{g_iter}); 
      fclose(g_fid_out); %close file

      g_fid_out = fopen([g_titlename '_trackfailed.txt'],'a'); %open file and create fid
      while g_fid_out<0
         fprintf('...waiting...')
         g_fid_out = fopen([g_titlename '_trackfailed.txt'],'a'); %open file and create fid
      end
...
if u_concatYN>0
   ...
   if u_figson==1||u_figson==3
      ...
      % Fo (hz)
      subplot(2,2,1)
      tmp_f0all=round([cncat1_f0_fo_prtfi(~isnan(cncat1_f0_fo_prtfi))...
          cncat1_f0_fo_audi(~isnan(cncat1_f0_fo_audi))...
          cncat1_f0_fo_shpi(~isnan(cncat1_f0_fo_shpi))]);
...
%% failed file list and copy into another folder

g_fid_out = fopen([g_titlename '_trackfailed.txt']);
badfiles = load([g_titlename '_trackfailed.txt']);
fclose(g_fid_out); %close file
...
if ~isempty(badfiles)
   ...
   for n=1:length(badfiles)
      ...
      tmpstatus=movefile(g_filename{badfiles(n)}, tmpstr);      
   end
end
